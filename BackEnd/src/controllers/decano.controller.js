...
export const deleteMateriaById = async (req, res) => {
  ...
  try {
    const pool = await getConnection();
    await pool.request().input("id", sql.Int, id).query(queries.deleteMateria);
    res.sendStatus(204);
  } catch (error) {
    res.send(error);
  }
...
export const updateStatusEstudiante = async (req, res) => {
  const { statusEstudiante, idEstudiante} = req.body;
  try {
    const pool = await getConnection();
    await pool
      .request()
      ...
      .query(queries.updateStatusEstudiantes);

    res.json(statusEstudiante);
  } catch (error) {
    res.send(error);
  }
};
